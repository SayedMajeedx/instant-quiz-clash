...
console.log(`Generating SQL sync for ${quizzes.length} quizzes in QUIZ_LIBRARY...`);

let sql = `-- Sync all QUIZ_LIBRARY quizzes directly to Supabase DB\n\n`;
...
fs.writeFileSync('scripts/sync_all_quizzes.sql', sql, 'utf8');
console.log('Saved scripts/sync_all_quizzes.sql.');
